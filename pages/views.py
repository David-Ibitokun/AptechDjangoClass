...
# --- Public Facing Views ---
# views.py (for homepage or base view)
...
def shop(request):
    ...
    # And then subcategories can be accessed via `category.subcategories.all()`
    ...
    context = {
        ...
        # You might still pass all_categories if needed elsewhere: 'all_categories': Category.objects.all(),
    }
...
# ... (other necessary imports like django.shortcuts, django.contrib.auth.decorators, etc.)
from django.contrib import messages # Make sure this is imported
from .models import Category, Product # Ensure Product and Category are imported
from .forms import ProductForm # Ensure ProductForm is imported
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            if request.user.is_authenticated:
                ...
                # This block should ideally not be hit due to @login_required,
                # but it's a safeguard for extreme cases (e.g., session invalidation race condition).
                ...
                return redirect('login') # Redirect back to login if somehow not authenticated
            ...
            if category_id:
                try:
                    ...
                    # Potentially re-render the form here or redirect with an error.
                    # For now, let's allow saving without category if it's nullable.
            ...
            # DEBUGGING START
            # Optional: Reload the product from the database to ensure the field persisted
            # from pages.models import Product as ProductModel # Avoid naming conflict with local Product
            # saved_product = ProductModel.objects.get(pk=product.pk)
            # print(f"DEBUG VIEWS: Product saved to DB. ID: {saved_product.id}, Creator from DB: {saved_product.creator}")
            # DEBUGGING END
